...
def fetchPth(path):
    """
    Load a (coords, colors, [normals,] labels) tuple from a .pth file.

    The Matterport3D pre-processing in this repo and the publicly shipped
    Matterport caches do not always store per-point normals (3-tuple files).
    To stay compatible with both ScanNet (4-tuple) and Matterport (3-tuple)
    we estimate normals via Open3D on the fly when they are missing.
    """
    try:
        data = torch.load(path, map_location='cpu')

        if not isinstance(data, (list, tuple)) or len(data) not in (3, 4):
            raise ValueError(
                f"Expected 3 or 4 elements in pth file, got "
                f"{len(data) if hasattr(data, '__len__') else type(data)}"
            )

        if len(data) == 4:
            coords, colors, normals, vertex_labels = data
        else:  # 3-tuple (Matterport pre-processing): no normals stored.
            coords, colors, vertex_labels = data
            normals = None
        ...
        if hasattr(colors, 'numpy'):
            colors = colors.numpy()
        if normals is not None and hasattr(normals, 'numpy'):
            normals = normals.numpy()

        if normals is None:
            # On-the-fly normal estimation for Matterport (.pth has no normals).
            # We deliberately use the cheap "+Z" orientation instead of the very
            # slow consistent-tangent-plane orientation: the downstream Sonata
            # teacher and the per-voxel geometry encoder both consume normals
            # through a learned channel, so absolute orientation is unimportant
            # as long as it is deterministic per-scene.
