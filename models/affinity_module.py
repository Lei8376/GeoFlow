...
class SonataXAffinityTrainer(nn.Module):
    def __init__(self, cfg, xdecoder_cfg, scene_config, device='cuda', use_lseg=True):
        ...
        # Paper (Table 7): 48 macro-negatives + 16 micro-negatives = 64 per anchor.
        self.num_negatives_per_anchor = 64
        self.num_macro_negatives = 48
        self.info_nce_temperature = 0.07   # InfoNCE Loss Temperature Coefficient
        # Geometry-Guided Pooling (inference). Defaults match the working repo
        # value (sharp affinity). The paper text says "concentrate weights on the
        # most similar points"; Table 7's "0.05 (1/20.0)" is the reciprocal
        # (temperature) convention. We expose both via cfg for ablation.
        self.pooling_iterations = int(getattr(cfg, 'pooling_iterations', 18))
        self.affinity_sharpen_factor = float(getattr(cfg, 'affinity_sharpen_factor', 20.0))
        self.pooling_knn = int(getattr(cfg, 'pooling_knn', 96))
    ...
    def lift_xdecoder_features(self, batch_data):
        ...
        feature_dim = 512
        counter = torch.zeros(scene_coords.shape[0], dtype=torch.float32, device=self.device)

        view_counts = torch.zeros(num_points, dtype=torch.long, device=self.device)
        for view_idx in range(len(imgs)):
            all_mask_this_view = (mask_2ds[:, 0] == view_idx)
            mask_2d = mask_2ds[all_mask_this_view][:, 1].bool().to(self.device)
        ...
        for view_idx in range(len(imgs)):
            ...
            # MEMORY: store accumulated per-view features on the CPU side so
            # that scenes with many views (Matterport often has > 200 views per
            # scene, vs. ~20 for ScanNet) do not blow up GPU memory. The chunk
            # loop below re-materialises them on GPU one slice at a time.
            feat_view_cpu = feature_2d.detach().cpu()
            logits_view_cpu = logits_pred_2d.detach().cpu()
            for i, global_idx in enumerate(point_indices_in_this_view):

                point_info[global_idx.item()].append({
                    "feature": feat_view_cpu[i],
                    "logits": logits_view_cpu[i],
                })
            counter[mask_2d] += 1
            del feature_2d
            del logits_pred_2d
            ...
            del img
            del feat_view_cpu, logits_view_cpu
        ...
        final_scene_confidence = torch.zeros(num_points, dtype=torch.float32, device=self.device)
        ...
        # Adaptive chunking: each chunk allocates
        #   chunk_size * M_max * (feature_dim + num_classes) * 4 bytes.
        # On a 10 GiB GPU and Matterport (M_max ~ 100), the hard-coded 50000
        # silently OOMs. We size the chunk to use at most ~25 % of free memory.
        M_max_global = max(len(v) for v in point_info.values()) if point_info else 1
        per_row_bytes = M_max_global * (feature_dim + num_classes) * 4
        if torch.cuda.is_available():
            free_b, _ = torch.cuda.mem_get_info(self.device)
        else:
            free_b = 4 * 1024 ** 3
        chunk_size = max(min(50000, int(free_b * 0.25) // max(per_row_bytes, 1)), 1024)
        for i in range(0, num_points_with_views, chunk_size):
            ...
            for j, global_idx in enumerate(chunk_indices):
                ...
                for k, view_data in enumerate(views_data):
                    padded_features_chunk[j, k] = view_data["feature"].to(self.device, non_blocking=True)
                    padded_logits_chunk[j, k] = view_data["logits"].to(self.device, non_blocking=True)
            ...
            top_k_features_norm = F.normalize(top_k_features_chunk.float(), dim=-1)
            fused_features_norm = F.normalize(fused_features_chunk.float(), dim=-1)
            semantic_consistency = (
                top_k_features_norm * fused_features_norm.unsqueeze(1)
            ).sum(dim=-1)
            semantic_consistency = ((semantic_consistency + 1.0) * 0.5).clamp(0.0, 1.0)
            semantic_consistency = semantic_consistency.mean(dim=1)

            view_ref = float(getattr(self.cfg, "geoflow_confidence_view_ref", 3.0))
            view_ref = max(view_ref, 1.0)
            view_confidence = (
                num_valid_views_chunk.squeeze(1).float() / view_ref
            ).clamp(0.0, 1.0)

            feature_w = float(getattr(self.cfg, "geoflow_confidence_feature_w", 0.7))
            view_w = float(getattr(self.cfg, "geoflow_confidence_view_w", 0.3))
            total_w = max(feature_w + view_w, 1e-6)
            confidence_chunk = (
                feature_w * semantic_consistency + view_w * view_confidence
            ) / total_w

            final_scene_features[chunk_indices] = fused_features_chunk
            final_scene_confidence[chunk_indices] = confidence_chunk
            del padded_features_chunk, padded_logits_chunk, validity_mask_chunk
            del top_k_features_chunk, top_k_scores_chunk, top_k_indices_chunk
            del top_k_features_norm, fused_features_norm, semantic_consistency
            del view_confidence, confidence_chunk
            del agreement_scores_chunk, sum_logits_chunk, avg_logits_chunk
            torch.cuda.empty_cache()
        ...
        self.last_lift_confidence = final_scene_confidence.detach()
        return (
    ...
    def sample_contrastive_pairs_hybrid(self, F_sonata_full, neighbor_indices):
        ...
        N_macro = self.num_macro_negatives
        N_micro = self.num_negatives_per_anchor - N_macro
        sim_matrix_neg = sim_matrix_global.clone()
    ...
    def forward(self, batch_data):
        ...
        # ------------------------------------------------------------------
        # FIX (train/inference consistency): the student must see the FULL
        # voxelized scene with the SAME 518-dim input used at inference time
        # (512 semantic + 6 geometric), then we gather per-voxel embeddings at
        # the sampled anchor/positive/negative points. The original code fed
        # only the sparse subset of sampled voxels with 512-dim features, which
        # (a) crashes / mismatches the 518-dim student input layer, and
        # (b) corrupts the sparse-conv receptive field (fragmented geometry),
        # producing a severe train/test distribution shift.
        # ------------------------------------------------------------------
        device = self.device
        scene_inds_reconstruct = scene_inds_reconstruct.to(device)

        # Per-voxel input features, identical construction to evaluate_scene().
        voxel_semantic_features = torch_scatter.scatter_mean(
            F_xdecoder_full.to(device), scene_inds_reconstruct, dim=0
        )  # [V, 512]
        raw_geom_features_full = scene_gauss_features[:, :6].float().to(device)
        voxel_geom_features = torch_scatter.scatter_mean(
            raw_geom_features_full, scene_inds_reconstruct, dim=0
        )  # [V, 6]
        voxel_features_input = torch.cat(
            [voxel_semantic_features, voxel_geom_features], dim=1
        )  # [V, 518]
        del voxel_semantic_features, voxel_geom_features, raw_geom_features_full

        s_input_student = ME.SparseTensor(
            features=voxel_features_input,
            coordinates=ME.utils.batched_coordinates([scene_coords_3d.to(device)]),
            device=device,
        )
        del voxel_features_input
        s_output_student = self.affinity_student(s_input_student)
        # ME stride-1 conv preserves input row order, so voxel embeddings align
        # with scene_coords_3d (and therefore with scene_inds_reconstruct).
        F_voxel_embed = s_output_student.F  # [V, 128]
        del s_output_student, s_input_student

        # Gather the per-voxel embedding for every sampled point.
        all_sample_indices, point_to_batch_map = torch.unique(
            torch.cat([anchor_idx, positive_idx, negative_idx.flatten()]),
            return_inverse=True
        )
        voxel_indices_for_samples = scene_inds_reconstruct[all_sample_indices]
        F_pred_on_samples = F_voxel_embed[voxel_indices_for_samples]
        del F_voxel_embed
        ...
        del F_xdecoder_full, F_pred_on_samples, F_pred_anchor, F_pred_positive, F_pred_negative, logits, labels
        torch.cuda.empty_cache()
        return loss
    ...
    def evaluate_scene(self, batch_data,vis_prefix = "scene0695_00"):
        K = self.pooling_knn
        affinity_sharpen_factor = self.affinity_sharpen_factor
        ...
        # MEMORY: per-voxel cosine affinity in CHUNKS. The original code built
        # both center_embeds=[V*K, D] and neighbor_embeds=[V*K, D] explicitly,
        # which OOMs on Matterport (V > 100K, K=96, D=128 -> ~10 GiB each).
        D = F_pred_embed.shape[1]
        affinity_values_2d = torch.empty(num_voxels, K, device=F_pred_embed.device)
        chunk_v = 8192
        for s in range(0, num_voxels, chunk_v):
            e = min(s + chunk_v, num_voxels)
            neigh_c = F_pred_embed[neighbor_indices[s:e]]          # [c, K, D]
            affinity_values_2d[s:e] = (F_pred_embed[s:e].unsqueeze(1) * neigh_c).sum(-1)
            del neigh_c
        del F_pred_embed

        affinity_weights = F.softmax(
            affinity_values_2d * affinity_sharpen_factor, dim=1
        ).flatten()
        del affinity_values_2d
        ...
        # Geometry-Guided Pooling: exactly T iterations of F <- A F  (paper Eq. 4).
        F_refined_voxel = voxel_features_input
        for _ in range(self.pooling_iterations):
