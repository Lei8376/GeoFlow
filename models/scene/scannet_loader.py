...
def readScanNetInfo(path, white_background, eval, llffhold=8, extensions=[".png", ".jpg"]):
    ...
    if intrinsic_path.exists():
        ...
        K = np.loadtxt(path.parent / "intrinsics.txt")
    ...
    if not os.path.exists(ply_path):
        # Since this data set has no colmap data, keep the placeholder point
        # cloud in memory. The validation loader uses the real .pth point cloud;
        # writing points3d.ply into the processed data directory breaks on
        # read-only dataset mounts.
        num_pts = 100_000
        print(f"Generating in-memory random point cloud ({num_pts})...")
