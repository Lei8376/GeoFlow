...
def _estimate_normals_from_points(points):
...
class ScannetLoaderFull(Point3DLoader):
    def __init__(
        ...
        for scene_data_path in tqdm(self.data_paths, desc="Loading scene configs"):
            scene_name = os.path.basename(scene_data_path).split('_vh_clean_2.pth')[0]
            ...
            temp_scene_config.scene_path = os.path.join(temp_scene_config.scene_path, scene_name)
            
            try:
                scene = Scene(temp_scene_config, resolution_scales=[2.0]) 
                views = scene.getTrainCameras(scale=2.0)
    ...
    def __getitem__(self, index_long):
        ...
        if scene_name in self.scene_cache:
            ...
            locs_in, feats_in, labels_in = torch.load(scene_data_path)
            ...
            original_ply_path = os.path.join(scene_path, "points3d.labels.ply")
            if not os.path.exists(original_ply_path):
                labels_root = getattr(self.scene_config.scene, "points3d_labels_root", None)
                if labels_root:
                    candidate = os.path.join(
                        labels_root, scene_name, f"{scene_name}_vh_clean_2.labels.ply"
                    )
                    if os.path.exists(candidate):
                        original_ply_path = candidate

            if os.path.exists(original_ply_path):
                original_xyz, _, original_labels, original_scan_normals = load_point_ply(
                    original_ply_path, islabel=True
                )
            else:
                print(
                    f"Warning: {scene_name} has no points3d.labels.ply; "
                    "estimating normals from the .pth point cloud."
                )
                original_xyz = locs_in
                original_labels = labels_in
                original_scan_normals = _estimate_normals_from_points(locs_in)
            point_features = np.concatenate([feats_in, original_scan_normals], axis=1)

        
            if self.scannet200:
                root_path = scene_data_path
                ...
                scannet_200_label_path = os.path.join(
                    ...
                    scene_data_path[:-15].split("/")[-1] + ".txt",
                )
        ...
        labels_in_clone = labels_in.copy()
        original_xyz = scene_data["original_xyz"]
...
class SceneBatchSampler(Sampler):
    ...
    def set_epoch(self, epoch):
        # Kept for API compatibility with the distributed sampler.
        random.seed(epoch)


class DistributedSceneBatchSampler(Sampler):
    """Scene-level batch sampler that shards whole scenes across DDP ranks.

    Each yielded item is the list of (scene, view) sample indices belonging to
    one scene (one optimizer step == one scene), and scenes are partitioned
    disjointly across ranks so the two 4090s do real data-parallel work instead
    of duplicating the same scenes.
    """

    def __init__(self, samples_list, num_replicas, rank, shuffle=True, max_indices_per_scene=0):
        self.samples_list = samples_list
        self.num_replicas = num_replicas
        self.rank = rank
        self.shuffle = shuffle
        self.epoch = 0
        self.max_indices_per_scene = int(max_indices_per_scene or 0)
        ...
        # Pad scene list so it divides evenly across replicas.
        self.num_scenes = len(self.scenes)
        self.num_scenes_per_replica = (self.num_scenes + self.num_replicas - 1) // self.num_replicas
        self.total_size = self.num_scenes_per_replica * self.num_replicas
    ...
    def __iter__(self):
        ...
        if self.shuffle:
            ...
            g.shuffle(scenes)
        if len(scenes) < self.total_size:
            scenes = scenes + scenes[: (self.total_size - len(scenes))]
        scenes = scenes[self.rank:self.total_size:self.num_replicas]
        for scene_name in scenes:
            indices = list(self.scene_to_indices[scene_name])
            if self.max_indices_per_scene > 0 and len(indices) > self.max_indices_per_scene:
                scene_seed = sum((i + 1) * ord(c) for i, c in enumerate(scene_name))
                g = random.Random((self.epoch + 1) * 1000003 + scene_seed)
                indices = g.sample(indices, self.max_indices_per_scene)
                indices.sort()
            yield indices

    def __len__(self):
        return self.num_scenes_per_replica
