...
class ScannetLoaderFull(Point3DLoader):
    def __init__(
        ...
        for scene_data_path in tqdm(self.data_paths, desc="Loading scene configs"):
            ...
            try:
                ...
                # Memory-saving: keep only the SceneDataset reference (which holds
                # CameraInfo metadata, NOT loaded images). Image tensors are
                # materialized lazily inside __getitem__ via views[view_idx].
                self.scene_datasets[scene_name] = views

                for i in range(len(views.camera_info)):
                    self.samples.append({
        ...
        print(f"Dataset created with {len(self.samples)} total views across {len(self.scene_datasets)} scenes.")
    ...
    def __getitem__(self, index_long):
        ...
        view_idx = sample_info["view_idx"]
        # Lazy-load the Camera (with its image tensor) on demand to keep RAM low.
        scene_dataset = self.scene_datasets[scene_name]
        view = scene_dataset[view_idx]       
        ...
        if scene_name in self.scene_cache:
            ...
            loaded = torch.load(scene_data_path)
            if isinstance(loaded, (list, tuple)) and len(loaded) == 4:
                locs_in, feats_in, original_scan_normals, labels_in = loaded
            elif isinstance(loaded, (list, tuple)) and len(loaded) == 3:
                # Matterport pre-processing ships (coords, colors, labels) with
                # no per-point normals. Estimate them with Open3D so downstream
                # voxelization / Sonata / GeoPurify all see a consistent 6-D
                # geometric tensor (xyz + normal) for the geometry condition.
                locs_in, feats_in, labels_in = loaded
                import open3d as o3d
                pcd = o3d.geometry.PointCloud()
                pcd.points = o3d.utility.Vector3dVector(
                    np.asarray(locs_in, dtype=np.float64)
                )
                ...
                original_scan_normals = np.asarray(pcd.normals, dtype=np.float32)
            else:
                raise ValueError(
                    f"Unexpected pth structure in {scene_data_path}: "
                    f"{type(loaded).__name__} len="
                    f"{len(loaded) if hasattr(loaded, '__len__') else 'NA'}"
                )
            ...
            # Bound scene_cache to a single entry. The SceneBatchSampler yields
            # all views of one scene back-to-back, so we only ever need the
            # current scene cached. Letting this dict grow unboundedly caused a
            # slow RAM leak (~50 MB/scene cumulative across ~200 scenes).
