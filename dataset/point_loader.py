...
def sa_create(name, var):
    """"""
    if SA is None:
        raise ImportError("SharedArray is required when memcache_init/use_shm is enabled.")
...
class Point3DLoader(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        if self.use_shm and SA is None:
            raise ImportError("SharedArray is required when memcache_init/use_shm is enabled.")
    ...
    def __len__(self):
        if "nuscene" in self.dataset_name and self.split != "train":
            return len(self.data_paths) // 5
        else:
