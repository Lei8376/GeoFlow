...
def main_worker(gpu, ngpus_per_node, argss, scene_config, xdecoder_cfg,dataset_name):
    ...
    max_eval_scenes = int(getattr(args, "max_eval_scenes", 0) or 0)
    if max_eval_scenes > 0:
        scene_ids_val = scene_ids_val[:max_eval_scenes]
    if main_process():
        logger.info(
...
def validate(val_loader, model,save_path=None):
    ...
    # Initialise so that an empty validation loader still returns sane values
    mIoU_2d_Base = 0.0
    mIoU_2d_Novel = 0.0
    ...
    with torch.no_grad():
        for i, batch_data in enumerate(val_loader):
            ...
            # Periodic checkpoint of raw per-class sums so that a crash near
            # the end of a long evaluation still leaves a usable partial result
            # for run/merge_eval.py. We write to a `.partial` file to keep it
            # distinguishable from the canonical end-of-run npz.
            try:
                if (i + 1) % 25 == 0 and save_path is not None:
                    split_idx = getattr(args, "split_idx", 0)
                    out_partial = os.path.join(
                        save_path, f"eval_raw_split{split_idx}.partial.npz"
                    )
                    np.savez(
                        out_partial,
                        intersection_all=intersection_meter_2d_All.sum,
                        union_all=union_meter_2d_All.sum,
                        target_all=target_meter_2d_All.sum,
                        scenes_processed=np.int64(i + 1),
                    )
            except Exception as _ckpt_err:
                if main_process():
                    logger.warning(f"partial ckpt write failed: {_ckpt_err}")
            ...
            import gc as _gc
            _gc.collect()
            # Return free heap to OS so RSS doesn't grow monotonically across
            # the long validation loop. Without this, glibc's malloc tends to
            # keep large per-scene transients in its arena and the process is
            # eventually OOM-killed mid-loop on the 49 GiB target box.
            try:
    ...
    # Persist raw (intersection, union, target) sums so multiple eval splits can
    # be merged into a single correct mIoU/mAcc via run/merge_eval.py.
    if save_path is not None:
        split_idx = getattr(args, "split_idx", 0)
        out_npz = os.path.join(save_path, f"eval_raw_split{split_idx}.npz")
        np.savez(
            out_npz,
            intersection_all=intersection_meter_2d_All.sum,
            union_all=union_meter_2d_All.sum,
        ...
        print(f"[eval] raw stats saved to {out_npz}")
    return (
        mIoU_2d_Base,
        mIoU_2d_Novel,
    )
